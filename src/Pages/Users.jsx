    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-5">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="primary" size="sm" onClick={() => setShowCreate(true)}><UserPlus size={14} />Create User</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 mb-4">
        ...
        <Card className="p-5 flex items-center gap-2.5">
          ...
          <div><div className="text-[22px] font-display font-bold text-slate-800">{inactive}</div><div className="text-xs text-slate-500">Inactive / Locked</div></div>
        </Card>
      </div>
      ...
      <Card>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                {['Staff Member', 'Role', 'Department', 'Joined', 'Status', 'Actions'].map((h) => (
            ...
            <tbody>
              {loading && (
                <tr><td colSpan={6} className="text-center text-sm text-slate-400 py-10"><Loader2 size={18} className="animate-spin inline-block mr-2" />Loading staff...</td></tr>
              )}
              {!loading && loadError && (
                <tr><td colSpan={6} className="text-center text-sm text-red-500 py-10">{loadError}</td></tr>
              )}
              {!loading && !loadError && filtered.length === 0 && (
                <tr><td colSpan={6} className="text-center text-sm text-slate-400 py-10">No staff match this filter.</td></tr>
              )}
                ...
                <tr key={s.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <img src={`https://i.pravatar.cc/64?u=${s.email}`} alt="" className="rounded-full flex-shrink-0 object-cover" style={{ width: 34, height: 34 }} />
                      <div>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ${roleBadgeStyle[s.roleType] || 'bg-slate-100 text-slate-600'}`}>{s.role}</span>
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-500 whitespace-nowrap">{s.dept}</td>
                  <td className="px-4 py-3 text-sm text-slate-500 whitespace-nowrap">{s.joined}</td>
                  <td className="px-4 py-3"><Badge tone={s.status === 'Active' ? 'active' : s.status === 'Locked' ? 'critical' : 'warning'}>{s.status}</Badge></td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5">
                      <button onClick={() => showToast('Edit user — coming soon', 'info')} className="w-7 h-7 flex items-center justify-center rounded-md border border-slate-200 text-slate-500 hover:bg-slate-100"><Edit2 size={12} /></button>
                      <button onClick={() => showToast('Permissions view — coming soon', 'info')} className="w-7 h-7 flex items-center justify-center rounded-md border border-slate-200 text-slate-500 hover:bg-slate-100"><Shield size={12} /></button>
                      <button onClick={() => showToast('Suspend user — coming soon', 'info')} className="w-7 h-7 flex items-center justify-center rounded-md border border-red-200 bg-red-50 text-red-600 hover:bg-red-100"><UserX size={12} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="flex items-center justify-between border-t border-slate-100" style={{ padding: '14px 18px' }}>
          <div className="text-[13px] text-slate-500">Showing 1–{filtered.length} of {filtered.length} staff members</div>
      ...
      {showCreate && <CreateUserModal onClose={() => setShowCreate(false)} onCreated={loadStaff} />}
    </div>
  )
}