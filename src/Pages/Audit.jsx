...
// These are the exact action label strings documented by the backend contract.
const actionIcons = {
  "Login Success": {
    icon: LogIn,
    bg: "bg-emerald-50",
...
function matchesFilter(log, filter) {
  ...
  if (filter === "Login") {
    return log.action?.startsWith("Login") || log.action?.startsWith("Account");
  }

  if (filter === "Record Access") {
    return log.action === "Record Viewed";
  }

  if (filter === "Upload") {
    return log.action === "Record Uploaded";
  }

  if (filter === "Failed") {
    return log.status === "Blocked" || log.status === "Failed";
  }
...
function downloadCSV(rows) {
  ...
  const body = rows
    .map((r) =>
      [r.time, r.user, r.role, r.action, r.target, r.ip, r.status]
...
export default function Audit() {
   ...
   *
   * Pagination and date filtering are handled server-side.
   * Search and the existing UI categories remain client-side
   * for the records currently loaded.
   */
  useEffect(() => {
    let cancelled = false;
  ...
  /*
   * Date changes should always return the user to page 1.
   */
  useEffect(() => {
    setPage(1);
  }, [dateFrom, dateTo]);
  ...
  /*
   * Search and the existing UI filter categories are applied
   * to the records currently loaded from the backend.
   *
   * Date filtering is intentionally NOT repeated here because
   * it is already handled server-side.
   */
  const filtered = useMemo(() => {
    return logs.filter((log) => {
      ...
      return matchesSearch && matchesFilter(log, activeFilter);
    });
  }, [logs, search, activeFilter]);

  /*
   * These metrics are temporarily derived from the loaded page.
   *
   * They will be replaced with /audit/logs/stats in the next
   * Audit integration step.
   */
  ...
  const metrics = [
    {
      ...
      bg: "bg-blue-50",
      color: "text-blue-600",
      value: String(logs.filter((l) => l.action === "Login Success").length),
    ...
    },
    {
      ...
      valueColor: failedLogins > 0 ? "text-red-600" : undefined,
      label: "Failed Logins",
    },
    {
      icon: UploadCloud,
  ...
  const firstResult = totalLogs === 0 ? 0 : (page - 1) * 10 + 1;

  const lastResult = Math.min(page * 10, totalLogs);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5 mb-4">
        {metrics.map((m) => (
          <Card key={m.label} className="p-4">
            <div
              className={`w-9 h-9 rounded-lg flex items-center justify-center mb-2.5 ${m.bg}`}
            >
      ...
      <Card>
          ...
          <table className="w-full">
            ...
            <tbody>
              ...
              {!loading && !loadError && filtered.length === 0 && (
                <tr>
                  <td
                    ...
                    className="text-center text-sm text-slate-400 py-10"
                  >
                    No events match your search, filter, or date range.
                  </td>
                </tr>
              )}
              ...
              {!loading &&
                ...
                filtered.map((log) => {
                  const a =
                    actionIcons[log.action] || actionIcons["Record Viewed"];
                  ...
                  return (
                    <tr
                      ...
                      className={`border-b border-slate-100 last:border-0 ${
                        isFailed ? "bg-red-50/40" : "hover:bg-slate-50"
                      }`}
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <div
                            className={`rounded-md flex items-center justify-center ${a.bg}`}
                            style={{
                              width: 22,
                              height: 22,
                            }}
                      ...
                      <td className="px-4 py-3 text-[13px] text-slate-500 whitespace-nowrap font-mono">
                        {log.target}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <Badge tone={statusTone[log.status] || "discharged"}>
                          {log.status}
                        </Badge>
                      </td>
                    </tr>
        ...
        >
          <div className="text-[13px] text-slate-500">
            Showing {firstResult}–{lastResult} of {totalLogs} events logged
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-xs px-3 py-1.5 rounded-md border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Previous
            </button>

            <span className="text-xs text-slate-500 px-2">Page {page}</span>
